# Full file — annotated explanation
...
* Interview note: order matters — body parsers before routes that need `req.body`.
...
* Interview tip: synchronous hashing blocks the event loop—acceptable in a small script but prefer `await bcrypt.hash(...)` in real apps.
...
* Interview note: show how middleware stacking works in Express (order: authenticate → authorize → handler).
...
* Interview note: you can add environment-specific behavior — e.g., include stack trace in development.
...
* Provide **example answers** (2–3 minute spoken version) that you can rehearse. Which would you like?
